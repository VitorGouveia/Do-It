...
export const NotesProvider: React.FC<NotesProviderProps> = memo(({ children }) => {
  ...
  useEffect(() => {
    ...
    const notesWithConvertedDates = parsedLocalStorageNotes.map((note) => ({
      ...note,
      createdAt: Temporal.Instant.from(note.createdAt)
    }));
  ...
  const createNote = useCallback(() => {
    ...
    localStorage.setItem("@notely:notes", JSON.stringify([
      note,
      ...notes
    ]));
  }, [notes])
  ...
  // helper fn that de-encrypts all the information in notes
  const getNotes = useCallback(() => {
    const decryptedNotes = notes.map(({ title, slug, text, ...note }) => ({
      title: decrypt(title || ""),
      slug: decrypt(slug || ""),
      text: decrypt(text),
      ...note
  ...
  return (
    <NotesContext.Provider
      value={{ notes, createNote, editTitle, deleteNote, editText, getNotes }}
    >
      {children}
    </NotesContext.Provider>
  ); 
